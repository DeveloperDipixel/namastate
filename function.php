...
function addScripts()
{
if(isset($_GET['foogallery_attachment_category']))
   {
	wp_enqueue_style('hello-bootstrap','https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css');
	wp_enqueue_style('hello-custom-css',get_template_directory_uri().'/assets/css/hello-css.css');
	wp_enqueue_script('hello-custom-js',get_template_directory_uri().'/assets/js/hello-js.js',['jquery'],false,true);
	wp_localize_script('hello-custom-js','fooajaxurl',admin_url('admin-ajax.php'));	
}
...
if( ! function_exists( 'layers_scripts' ) ) {
	function layers_scripts(){
		...
		*/
		...
		wp_enqueue_script(
			LAYERS_THEME_SLUG . '-three' ,
			get_template_directory_uri() . '/assets/js/three.min.js',
			array(),
			LAYERS_VERSION
		); // Sticky-Kit

		wp_enqueue_script(
			LAYERS_THEME_SLUG . '-gsap' ,
...
function useAjaxforFooGallery()
{
	include_once get_template_directory().'/core/foo-ajax.php';
	wp_die();
}
add_action('wp_ajax_nopriv_foo','useAjaxforFooGallery');
add_action('wp_ajax_foo','useAjaxforFooGallery');